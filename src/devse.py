...
def generate_svg_badge_with_embedded_logo(
    ...
    badge_color="#000000",
    text_color="#ffffff",
    logo_file_path="logo/devse.svg"  # 반드시 로컬 SVG 파일
):
    # SVG 배지 사이즈 설정
    badge_width = 90
    badge_height = 28
...
<svg xmlns="http://www.w3.org/2000/svg" width="{badge_width}" height="{badge_height}" role="img" aria-label="{label} badge">
  ...
  <image href="{logo_data_uri}" x="{logo_x}" y="4" height="{logo_height}"/>
...
if __name__ == "__main__":
    generate_svg_badge_with_embedded_logo(
        output_path="badge/DevSe_badge.svg",
        label="DevSe",
        badge_color="#000000",
        text_color="#ffffff",
        logo_file_path="logo/devse.svg"
    )
